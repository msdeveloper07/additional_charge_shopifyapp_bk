...
class ProductController extends Controller
{
    public function createHandlingProduct(Request $request) {
        ...
        $percent_value = DB::table('fees')->where('store', $host)->where('fees_type','percent')->value('price');

        $decimalPrice = sprintf('%.2f', $request->cart_total / 100);
        $calculate_price = $decimalPrice*$percent_value/100;
        $fees_preice = round($calculate_price, 2);
        // print_r($fees_preice);   die;

        $fees_name = DB::table('fees')->where('store', $host)->where('fees_type','percent')->value('fees_name');
        $product_name = $fees_name.' '.$fees_preice.'%';
        ...
        $product['product_type'] = "Handling Fees";
        $product['handle'] = 'handling_fees';
        $product['template_suffix'] = '';
        $product['status'] = 'active';
        //$product['published_at'] = '';
        //$product['published'] = false;
        $product['tags'] = array('Handling Fees');
        ...
        //echo"<pre>"; print_r($res_arrayV);    die;
        ...
        return $this->updateProductVariant($product_id,$variant_id,$fees_preice,$host,$shop,$access_token,$product_name);  
    }

    public function updateProductVariant($product_id,$variant_id,$fees_preice,$host,$shop,$access_token,$product_name){
        $product['id'] = $product_id;
        $product['title'] = $product_name;
        $variant['id'] = $variant_id;
        $variant['product_id'] = $product_id;
        $variant['title'] = $product_name;
        ...
        $variant['old_inventory_quantity'] = 25;
